...
const Dashboard = () => {
	...
	return (
		<main className='lg:container px-8 mx-auto pb-8 mt-8 xl:mt-20 font-poppins tracking-widest flex flex-col xl:flex-row text-white'>
			<h1 className='sr-only'>{'Covid statistics'}</h1>
			<div className='mt-20'>
				<img src={hero} className='pr-8 hidden xl:block ' alt='People fighting against covid' />
				<img src={logo} className='px-8 block xl:hidden' alt='Covid Stats' />
			</div>

			<div className='flex flex-grow flex-col m-8'>
				<img src={logo} className='p-8 hidden xl:block' alt='Covid Stats' />

				<Bar date={last_updated_date} country={country} setCountry={handleCountryChange} countries={countries} />
				...
				<div className='flex flex-col xl:flex-row gap-4'>
					<VaccineCard data={people_vaccinated} subdata={people_vaccinated_per_hundred} type={'single'} />
					<VaccineCard data={people_fully_vaccinated} subdata={people_fully_vaccinated_per_hundred} type={'double'} />
					<VaccineCard data={total_vaccinations} subdata={new_vaccinations} type={'total'} />
				</div>
			</div>
		</main>
	);
